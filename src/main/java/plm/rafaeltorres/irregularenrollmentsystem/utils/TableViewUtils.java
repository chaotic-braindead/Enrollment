    ...
    public static void generateEditableTableFromResultSet(TableView tbl, ResultSet rs, String[] args){
        try{
            ...
            for(int i = 0; i < rs.getMetaData().getColumnCount(); ++i){
                ...
                Map<String, String> map = Map.ofEntries(Map.entry("STUDENT_NUMBER", "STUDENT_NO"), Map.entry("LAST_NAME", "LASTNAME"), Map.entry("FIRST_NAME", "FIRSTNAME")
                , Map.entry("GENDER", "GENDER"), Map.entry("BIRTHDAY", "BDAY"), Map.entry("ADDRESS", "ADDRESS"), Map.entry("CELLPHONE_NUMBER", "CP_NUM"),
                        Map.entry("EMAIL", "EMAIL"), Map.entry("COURSE_CODE", "COURSE_CODE"), Map.entry("STATUS", "STATUS"));
                col.setOnEditCommit(new EventHandler<TableColumn.CellEditEvent>() {
                    @Override public void handle(TableColumn.CellEditEvent t) {
                        ...
                        try{
                            ...
                            PreparedStatement ps = conn.prepareStatement("UPDATE " + args[0] + " SET " + map.get(rs.getMetaData().getColumnName(j+1)) + " = ? "+ "WHERE "+ args[1] +" = ?");
